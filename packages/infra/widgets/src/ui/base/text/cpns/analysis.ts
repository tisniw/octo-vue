/**
 * 文本分析模块 - 完整实现 pretext 的 analysis.ts
 * 处理文本分段、空白规范化、CJK 字符识别等
 */
...
let sharedWordSegmenter: any = null
let segmenterLocale: string | undefined

function getSharedWordSegmenter(): any {
...
function getLastCodePoint(text: string): string | null {
  ...
  if (last <= 0) return text.slice(Math.max(last, 0))
  
  const lastCodeUnit = text.charCodeAt(last)
  if (lastCodeUnit < 0xDC00 || lastCodeUnit > 0xDFFF) return text.slice(last)
  
  const maybeHigh = last - 1
  if (maybeHigh < 0) return text.slice(last)
  
  const highCodeUnit = text.charCodeAt(maybeHigh)
  return highCodeUnit >= 0xD800 && highCodeUnit <= 0xDBFF ? text.slice(maybeHigh) : text.slice(last)
}
